...
- **Verdict-only expensive reviewers + cheap implementers (Decision 9):** Expensive review lanes (`codex-reviewer`, `reviewer`, `ag-opus`) post verdicts and findings only. They never implement fixes directly. Gating reviews use either `reviewer` or `ag-opus` (preferring `reviewer` for the biggest decisions; `ag-opus` quota under research by PacingGovernor). Cheaper lanes (`task`/Flash, `ds-task`, Super Build) implement the fixes, sync and merge forward, and request a delta review (`delta-from: <sha>`) from the same reviewer. If Codex hits `usage_limit_reached`, the review role falls back directly to `reviewer`/`ag-opus`, never to Flash.
