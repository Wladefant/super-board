...
- **Closing keywords only fire on the default branch.** `Closes` / `Fixes` / `Resolves #N` close the linked issue when — and only when — the commit reaches the repository's **default** branch. A pull request merged into `staging`, or any other non-default base, closes nothing: GitHub creates no closing link and fires no close event, so the issue stays open and its card silently stays where it is. On a board whose work merges outside the default branch (PolySimulator's default is `main` while every pull request targets `staging`), write `Refs #N` plus the full issue URL and close the issue explicitly once its acceptance criteria are evidenced; that is the closure-normalizer workaround tracked in [#70](https://github.com/Wladefant/super-board/issues/70), and every closure it performs is audited back to the merged pull request rather than trusted. Where keywords do fire, they fire even for a partial pull request — use one only when every acceptance criterion is met, otherwise `Part of` plus the full issue URL.
