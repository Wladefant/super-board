...
class DerivationRuleTests(unittest.TestCase):
    ...
    def test_this_release_is_what_the_rule_produces(self) -> None:
        # 2.3.0 shipped and was never tagged, so the tag still says v1.2.0 and
        # still does not vote. This release restores the retired-status safety
        # gate on installed trees and adds no contract, so it is the patch branch.
        current, _reasoning = reconcile_current_release("2.3.0", "2.3.0", "v2.3.0", "v1.2.0")
