...
- **Bounded Concurrency & Host RAM Hygiene:** the worker-count floors above are bounded by measured host resources. Measure host RAM and disk before every spawn with `python C:/Users/wkiri/.veyyon/workflows/host_status.py --json` (or `veyyon host --json`): at **>=95%** (no_spawn) spawn no new workers; at **>=90%** (wait) wait; at **>=85%** (reap) reap finished dev servers and idle processes before adding tasks (disk floor evaluations and reclamation triggers are delegated to `host_status.py`). Every lane MUST terminate any dev server it launches (`node.exe`, `next dev`, `start-server`) and verify the port is released before yielding.
...
- **ChatGPT Web Ban (2026-09-26):** The unofficial ChatGPT Web bridge (`codex-chatgpt-web` on 127.0.0.1:17841, `chatgpt-web/*` models, `web-task`/`web-thinker` lanes) is permanently banned and disabled. Rapid automated web request volume (up to 28 requests in 7 minutes and 57/hr with up to 70k tokens) through consumer web automation triggered OpenAI anti-abuse invalidation, causing the entire ChatGPT Pro subscription access to be removed. All `chatgpt-web/*` models are stripped from routing ladders, fallback chains, and subagent configs. Only the official OpenAI Codex OAuth API path (`openai-codex/*`) is permitted, governed strictly by established rate and window limits.
