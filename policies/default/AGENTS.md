...
- **Review Discipline & Economy:** (1) No review is dispatched while CI is red; (2) Never re-review content already approved (existing patch-id and stripped diff sha256 content binding stays); (3) Content changes require only a delta review (`delta-from: <sha>`), and only if the PR still requires review; (4) Sync merges (`origin/staging` forward) and docs/evidence pushes leave diff content unchanged and never invalidate reviews; (5) A sync lane's own approval is self-approval and never counts.
