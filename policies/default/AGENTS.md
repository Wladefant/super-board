...
- **Nested Subagent Economy:** Nested sub-agents default to cheap lanes (Flash `task`, or `spark` for small reviews). An Opus or `reviewer` child is only allowed for a high-risk review (money, auth, concurrency, migration).
