...
- **Reset-Aware Routing & Allowance-Safe Dispatch:**
  ...
  - **Use every allowance, every day (operator 2026-09-25, [#210](https://github.com/Wladefant/super-board/issues/210)):** before each wave read `veyyon usage --json` and spend the windows that reset soonest with the most headroom first. `model_routing.py` encodes this ladder; the matching `config.yml` roles are:
    - **Antigravity daily windows are per model family** (Gemini, Anthropic, OpenAI; each resets daily and was reported at 0% used for Claude/GPT). Medium-risk reviews and deep reasoning go to `ag-opus` (`google-antigravity/claude-opus-4-6`) while its family window has at least 10% left; `ag-sonnet`/`ag-gpt` take bounded medium work. These run before paid direct Anthropic and before Codex pro.
    - **Weekly windows are paced, not capped.** Pace headroom = remaining fraction ÷ remaining time fraction of the window (1.0 = linear spend to exactly 0 at reset). Codex pro below 1.0x is held back (emergency-only behind Antigravity Claude and Anthropic slack), because it would run dry mid-week; within 48h of reset at 1.25x or more it is promoted ahead of every other strong lane, because that surplus would expire unused. The free Codex account is unaffected.
    - **Direct Anthropic is the orchestrator's weekly budget.** Workers use it only when its 7-day window runs at 1.10x pace or more (slack the orchestrator cannot consume); low/medium work takes it only within 48h of reset at 1.25x or more. Stale or unknown Anthropic data never counts as slack. Below that, workers draw on it only when Antigravity Claude and Codex are both unavailable.
    - **DeepSeek V4.1 Flash is the cheap worker overflow** (`ds-task`: direct `deepseek/deepseek-flash:high`, falling back to `openrouter/deepseek/deepseek-v4.1-flash`, provider-routed to DeepInfra in `models.yml`). It takes routine execution when Gemini Flash is in cooldown and Codex pro is held back by pace, under an Opus/Fable orchestrator. Terminal-Bench 4.0: 26.8% vs Gemini 3.8 Flash high 19.7%. Never pin OpenRouter to `@together` (blocked by the account data policy).
