...
- **Verdict-Only Reviewers & Cheap Implementers (2026-09-26, Decision 9):** split reviewer and implementer roles. Expensive review lanes (`codex-reviewer`, `reviewer`) post verdicts and findings only; gating reviews route to `reviewer` (direct Anthropic Opus 5), never `ag-opus` (Antigravity weekly pool low). Cheaper lanes (`task`/Flash, `ds-task`) implement the fixes, sync and merge, then request a delta review (`delta-from: <sha>`) from the same reviewer.
- **Codex 429 Fallback (2026-09-26):** when Codex hits `usage_limit_reached`, `record_429` marks `openai-codex` exhausted until window reset; Case B1 High-Risk Review falls directly to `reviewer` (direct Anthropic Opus 5; avoid `ag-opus`), never to Flash (enforced in `model_routing.py` lines 1426-1454 by GoWeeklyPacing).
