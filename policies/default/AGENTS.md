...
- **Reset-Aware Routing & Allowance-Safe Dispatch:**
  ...
  - **Use every allowance, every day (operator 2026-09-25, [#210](https://github.com/Wladefant/super-board/issues/210)):** before each wave read `veyyon usage --json` and spend the windows that reset soonest with the most headroom first. `model_routing.py` encodes this ladder; the matching `config.yml` roles are:
    - **Antigravity daily windows are per model family** (Gemini, Anthropic, OpenAI; each resets daily and was reported at 0% used for Claude/GPT). High-risk reviews go to `ag-opus` (`google-antigravity/claude-opus-4-6`) while its family window has at least 10% left; `ag-sonnet`/`ag-gpt` take bounded medium work. Opus/ag-opus is used **only for high-risk review**; implementation, reasoning and medium/low review never route to Opus. These run before paid direct Anthropic and before Codex pro.
    - **Weekly windows are paced, not capped.** Pace headroom = remaining fraction ÷ remaining time fraction of the window (1.0 = linear spend to exactly 0 at reset). Codex pro is throttled only when headroom falls below 0.90 AND at least 50% of the window is consumed (tolerance band prevents a single early-week task from locking Codex out); within 48h of reset at 1.25x or more it is promoted ahead of every other strong lane. The free Codex account is unaffected.
    - **Direct Anthropic is the orchestrator's weekly budget.** Workers use it only when its 7-day window runs at 1.10x pace or more (slack the orchestrator cannot consume) AND the 5-hour window is below 80% used. Low/medium work takes it only within 48h of reset at 1.25x or more. Stale or unknown Anthropic data never counts as slack. Below that, workers draw on it only when Antigravity Claude and Codex are both unavailable.
    - **Every new fallback pair crosses a provider/credential boundary** (Antigravity to Codex or DeepSeek; Anthropic to Codex; Codex to Anthropic or DeepSeek). When the sidecar dies, every Antigravity model fails together, so same-provider fallback gives no protection.
    - **DeepSeek V4.1 Flash is the cheap worker overflow** (`ds-task`: direct `deepseek/deepseek-flash:high`, falling back to `openrouter/deepseek/deepseek-v4.1-flash`). It takes routine execution when Gemini Flash is in cooldown and Codex pro is held back by pace. Terminal-Bench 4.0: 26.8% vs Gemini 3.8 Flash high 19.7%. DeepSeek V4.1-Pro (`deepseek/deepseek-pro:high`, TB4 ~38%) is used as the cross-provider fallback for high-risk strong-ladder paths.
    - **Chinese-model worker slots ([#214](https://github.com/Wladefant/super-board/issues/214)):** Z.AI GLM Coding Plan (`zai/glm-5.3:high`, TB4 41.9%; `zai/glm-5.3-flash:high`, TB4 32.8%) and MiniMax Token Plan (`minimax-code/minimax-m3`, TB4 2.0%) are credential-gated: set `ZAI_API_KEY` or `MINIMAX_API_KEY` to activate them with no code change. When Z.AI is active, GLM-5.3 ranks above DeepSeek Flash for medium-risk implementation overflow. MiniMax is triage/bulk/docs only.
    ...
    - High-risk work (money, auth, concurrency, migration) never falls back to Flash, DeepSeek or a free model alone.
