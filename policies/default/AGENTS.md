...
- **Reset-Aware Routing & Allowance-Safe Dispatch:**
  ...
  - **Use every allowance, every day (operator 2026-09-25, [#210](https://github.com/Wladefant/super-board/issues/210); routing ruling recorded in [#210 comment 5832626598](https://github.com/Wladefant/super-board/issues/210#issuecomment-5832626598)):** before each wave read `veyyon usage --json` and spend the windows that reset soonest with the most headroom first. `workflows/portable/model_routing.py` encodes these ladders, and `routing_smoke_test.py` asserts them:
    - **Paid Anthropic (Fable, Opus) serves Main and high-risk reviews only.** It is never a worker primary and never a worker fallback while any cheap tier has headroom. Implementation, reasoning, deep context and medium/low review reach Fable only as the last rung of their ladder, when every cheap tier is out, and only from slack (see the reserve rule below); otherwise the pay-per-token DeepSeek V4 Pro last resort is used. The orchestrator reserve is never drawn by a worker. The deep-context ladder (over 180k tokens, or a deep-context task) puts the 1M-window tiers first (Gemini Pro, DeepSeek V4 Pro, MiniMax-M3 when credentialed), then every cheap tier whose verified window holds the context: GLM-5.3 (131k), Codex Astra medium (272k), Codex Fast (400k) on pace, `ag-opus` (250k), and Codex ahead of pace. Fable comes after all of them. The high-risk review ladder is: expiring Codex surplus, `ag-opus`, Fable on slack, Codex Astra medium on pace, Fable from the reserve, Codex ahead of pace, Gemini Pro, DeepSeek V4 Pro.
