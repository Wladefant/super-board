...
- **Bounded Concurrency & Host RAM Hygiene:** the worker-count floors above are bounded by measured host RAM. Measure host RAM before every spawn with `python C:/Users/wkiri/.veyyon/workflows/host_status.py --json` (or `veyyon host --json`): at **>=95%** (no_spawn) spawn no new workers; at **>=90%** (wait) wait; at **>=85%** (reap) reap finished dev servers and idle processes before adding tasks. Every lane MUST terminate any dev server it launches (`node.exe`, `next dev`, `start-server`) and verify the port is released before yielding.
...
- **DeepSeek Usage (2026-09-26):** the `ds-task` lane is used regularly for bounded implementation, cross-family advisory reviews, and overflow when Flash or Go are short. Never for first passes on money/auth/migration/concurrency or for gating reviews (enforced in `model_routing.py` via GoWeeklyPacing).
- **ChatGPT Web Full Usage (2026-09-26):** every chatgpt.com model's allowance is used each window, with request-based self-counting; GPT-6 Pro gets massive multi-hour tasks. ChatGPT Pro ($200/mo) caps GPT-6 Pro (`chatgpt-web/pro`) at 200 req/7d rolling (~28.5/day), reserved strictly for massive monolithic multi-hour tasks; all other modes (extra-high, high, medium, light) are unlimited. Counter tracked in `~/.veyyon/run/chatgpt-web-usage.json`, limits published at `~/.veyyon/run/chatgpt-web-limits.json` (owned by Gpt6ProPolicy #242 and GoWeeklyPacing in `model_routing.py`).
- **Codex Allocation (2026-09-26):** stays as before — a passive request counter only, with no share increase.
- **Invalid Agent Names (2026-09-26):** a session that dispatches an unknown agent name must not silently fall back to a similarly named expensive lane. The dispatch preamble must state the routine default (`task`) explicitly. The harness fails closed with `Unknown agent "<name>". Available: ...` (`task/index.ts:748-750`) rather than auto-substituting.
- **Burn-Rate Pacing & Antigravity Routing (2026-09-26):** Run `python workflows/portable/model_routing.py pace --json` to inspect per-window burn rates and action recommendations (`ok`, `throttle`, `spend-more`). Antigravity partner pools are tracked per account (cite `packages/ai/src/usage/google-antigravity.ts:434-441, 180-192, 74-79`): `brendmark` is throttled when its weekly cap shared by Claude and GPT models is over pace, while `ag-opus` routes to the healthy unthrottled account (`brandy.sengco` daily window) holding >= 10% reserve. Gating roles (`reviewer`, `ag-opus`) NEVER lead with `chatgpt-web`.
- **Role Pin Updates Without Restart (2026-09-26):** When `config.yml` agent pins or model roles are updated, running sessions pick them up via `/reload-config` (`packages/coding-agent/src/slash-commands/builtin-setup.ts:873-876` calling `Settings.reloadConfig()`) without requiring a full session restart ([Wladefant/veyyon#110](https://github.com/Wladefant/veyyon/issues/110)).
...
- **Fix, Don't Explain (2026-09-26):** when a lane or session misroutes or errs, fix the steering source (stale worktree, workflow copy, agent description, memory) instead of only explaining it. Put the fix at the source immediately.
...
- **Adopt-or-Reject (2026-09-26):** Every recommendation/requirement is adopted (shipped + wired into default use, cite adopted-at) or rejected (recorded rule). Never close a parent with open sub-issues; enforced by close-point guards: no closing keywords on parent PRs, plus a guard in ledger.py, the project adapters and GitHub-ops skills that refuses to close a parent with open sub-issues (owned by AdoptOrRejectRule2).
...
- **Verdict-Only Reviewers & Cheap Implementers (2026-09-26, Decision 9):** split reviewer and implementer roles. Expensive review lanes (`codex-reviewer`, `reviewer`, `ag-opus`) post verdicts and findings only; gating reviews use either `reviewer` or `ag-opus` (preferring `reviewer` for the biggest decisions; `ag-opus` routes to the healthy `brandy.sengco` account with >= 10% reserve holding per PacingGovernor). Cheaper lanes (`task`/Flash, `ds-task`) implement the fixes, sync and merge, then request a delta review (`delta-from: <sha>`) from the same reviewer.
- **Codex 429 Fallback (2026-09-26):** when Codex hits `usage_limit_reached`, `record_429` marks `openai-codex` exhausted until window reset; Case B1 High-Risk Review falls directly to `reviewer`/`ag-opus`, never to Flash (enforced in `model_routing.py` lines 1426-1454 by GoWeeklyPacing).
...
- **Clickable Links Mandatory (2026-09-26):** Every issue and PR reference in GitHub reporting MUST be a full clickable URL (`https://github.com/<owner>/<repo>/issues/<N>` or `pull/<N>`). Never emit bare `#N` or plain text references.
- **Source-Verified Reporting (2026-09-26, Decision 13):** Every GitHub object a lane reports (PR number, head SHA, comment URL) and every log ID is verified against the source (`gh pr view --json headRefName,headRefOid,commits`, `git ls-remote`, or a log search) before the orchestrator relays it or acts on it. Never state or relay an unverified identifier.
...
6. **Encode Immediately (2026-09-26):** verify that any rule derived from the failure is committed to policy, skill, or code in the same cycle; never rely on Mnemopi memory alone.
