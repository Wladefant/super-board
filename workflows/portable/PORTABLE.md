...
| **Decision Resolution (`DEC-*`)** | Human Operator Required | Pauses at `wait`; requires human response via GitHub issue comment or `decision_workflow.py reply`. A refused reply never resolves and never closes the question: it is audited as a rejected input while the question stays open for a later verified answer. |
| **Legacy Decision Recovery (`recover`)** | Explicit Operator Command, Fail-Closed | `decision_workflow.py recover <DEC-ID> --actor <handle>` reopens only an unanswered question whose legacy `rejected` status came from an audited refused input. It refuses answered, ledger-resolved, historyless, ambiguous or out-of-scope records; it retains audit, question binding, decision blockers and authorization; and it never writes an answer. |
