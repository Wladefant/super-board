...
| **`model_routing.py`** | `ImplementHarnessRouting` / `ImplementSubscriptionRouter` | Capability-first, reset-aware model selection, near-reset Codex promotion, per-window burn-rate pacing (`pace [--json]`), and Antigravity partner pool account routing (`brandy.sengco` vs `brendmark`). | `python model_routing.py [pace [--json]] [--task-type <type>] [--risk-level <level>] [--emit-dispatch] [--adapter veyyon\|file\|direct] [--balance-file <file>]` | `NormalizedBalanceSnapshot` | `HarnessDispatchPacket` or `WindowBurnPace` (JSON) |
...
python workflows/portable/test_recurrence_guard.py
```
...
| **Decision Resolution (`DEC-*`)** | Human Operator Required | Pauses at `wait`; requires human response via GitHub issue comment or `decision_workflow.py reply`. A refused reply never resolves and never closes the question: it is audited as a rejected input while the question stays open for a later verified answer. Answer time provenance is split: `answer.comment_created_at` is the comment's own creation time, written only on an API-verified `ingest`/`sync` and never from a caller argument, while `answer.answered_at` is ingest audit that routinely lags the comment. Once answered the decision is terminal — only the exact authenticated comment replays (idempotently), and any later or edited comment is refused as a rejected input without rewriting the answer, its timestamps, or the request it unblocked. |
| **Legacy Decision Recovery (`recover`)** | Explicit Operator Command, Fail-Closed | `decision_workflow.py recover <DEC-ID> --actor <handle>` reopens only an unanswered question whose legacy `rejected` status came from an audited refused input. It refuses answered, ledger-resolved, historyless, ambiguous or out-of-scope records, and independently requires the ledger to show a non-terminal request holding an unresolved decision entry for that exact id: an all-terminal, mismatched, missing or ambiguous binding is refused, so finished work is never re-blocked or pulled back into the sync window. A mixed record recovers only its open half and leaves every terminal record byte-identical. It retains audit, question binding, decision blockers and authorization; and it never writes an answer. |
