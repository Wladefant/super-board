...
| `pi.on("tool_call", cb)` | Hook | Native extension API capability. | **Not registered by Telegram:** tool-call validation and approvals were removed at operator request. Native permissions are separate. |
...
| **4. Approve tool call** | Native capability, outside Telegram scope | Removed; no Telegram hook or approval producer | Not exposed through this harness | **Removed at operator request.** Stale Telegram tool-approval callbacks never execute or authorize work. |
